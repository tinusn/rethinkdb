...
// Definitions: https://github.com/tinusn/rethinkdb/

declare module rethinkdb {
	/**
	 * The top-level ReQL namespace.
	 */
	interface r extends RqlDo, Wait {
		/**
...
n: n requests will be made
	...
	interface Wait {
		 ...
		 */
		wait(options?: WaitOptions): Operation<WaitResult>;
	}

	interface WaitOptions {
		/**
		 * a string indicating a table status to wait on before returning, one of ready_for_outdated_reads, ready_for_reads, ready_for_writes, or all_replicas_ready. The default is ready_for_writes.
		 */
		waitFor?: string;
		/**
		 * a number indicating maximum time to wait for in seconds before returning. The default is no timeout.
		 */
		timeout?: number;
	}
	...
	interface WaitResult {
		/**
		 * an integer indicating the number of tables waited for. It will always be 1 when wait is called on a table, and the total number of tables when called on a database.
		 */
		ready: number;
	...
	interface Admin<T> {
		 ...
		 */
		reconfigure(options: ReconfigureOptions): Operation<ReconfigureResult>;
	}

	interface ReconfigureOptions {
		/**
		 * the number of shards, an integer from 1-32. Required.
		 */
		shards: Shard[];
		/**
	...
	interface ReconfigureResult {
		 ...
		 */
		config_changes: OldValNewVal<TableConfig>[];
		/**
	...
	interface TableShardStatus {
		/**
		 * true if the table is ready for read queries with the useOutdated flag set to true.
		 */
		ready_for_outdated_reads: boolean;
		/**
		 * true if the table is ready for read queries with current data (with the useOutdated flag set to false or unspecified).
		 */
		ready_for_reads: boolean;
		/**
		 * true if the table is ready for write queries
		 */
		ready_for_writes: boolean;
		/**
		 * true if all backfills have finished.
		 */
